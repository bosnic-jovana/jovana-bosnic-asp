...
namespace Resturant.Implementation.UseCases.Queries
{
    public class EFGetOrdersQuery : EFUseCase, IGetOrdersQuery
    {
        ...
        public string Name => "Search orders using Entity Framework.";

        public IEnumerable<OrderDto> Execute(int user)
        {
            var query = Context.Orders
                            .Include(x => x.OrderItems)
                            .ThenInclude(x => x.Pricelist)
                            .ThenInclude(x => x.Specialty)
                            .Where(x => x.UserId == user && x.IsActive);

            if (query == null)
            {
                throw new EntityNotFoundException(nameof(User), user);
            }
